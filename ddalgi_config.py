# ddalgi_config.py

import os
from dotenv import load_dotenv

# .env 파일에 적힌 값들을 파이썬으로 불러옵니다.
load_dotenv()

class Config:
    # 1. os.getenv()를 이용해 .env의 값을 꺼내옵니다.
    db_user = os.getenv('DB_USER', 'root')
    db_password = os.getenv('DB_PASSWORD', '') # 비밀번호가 없으면 빈 칸 처리
    db_name = os.getenv('DB_NAME', 'ddalgi')
    db_endpoint = os.getenv('DB_ENDPOINT', 'localhost') 
    
    SQLALCHEMY_ENGINE_OPTIONS = {
        'connect_args': {
            'ssl': {
                'check_hostname': False # 인증서 깐깐한 검사는 패스하되, SSL 암호화 통신은 켜는 마법의 옵션
            }
        }
    }
    
    # 2. 꺼내온 값들을 퍼즐처럼 조립해서 DB 주소를 만듭니다. (localhost 대신 {db_endpoint} 사용)
    db_endpoint = os.getenv('DB_ENDPOINT', 'localhost')
    SQLALCHEMY_DATABASE_URI = f'mysql+pymysql://{db_user}:{db_password}@{db_endpoint}:3306/{db_name}'
    SQLALCHEMY_TRACK_MODIFICATIONS = False
    
    # 3. MQTT 설정도 동일하게 적용합니다.
    MQTT_BROKER = os.getenv('MQTT_BROKER', 'localhost')
    MQTT_PORT = 1883
    
    # 4. AWS S3 설정
    AWS_ACCESS_KEY_ID = os.getenv('AWS_ACCESS_KEY_ID')
    AWS_SECRET_ACCESS_KEY = os.getenv('AWS_SECRET_ACCESS_KEY')
    AWS_REGION = os.getenv('AWS_REGION', 'ap-southeast-2')
    S3_BUCKET_NAME = os.getenv('S3_BUCKET_NAME')